...
(function(config) {
    ...
    function readSession(key) {
        var fromSession = readStorage(window.sessionStorage, key);
        if (fromSession != null) {
            return fromSession;
        }
        return readStorage(window.localStorage, key);
    }
    if (!config || !config.userInfoKey) {
        return;
    }
    if (readSession(config.userInfoKey) != null) {
        return;
    }
    if (config.loginShellUrl) {
        window.location.replace(config.loginShellUrl);
    }
})({{{ssoGuardJson}}});
</script>
{{/ssoGuard.enabled}}
