...
const ssoGuard = {
    enabled: Boolean(process.env.LOGIN_SHELL_URL),
    loginShellUrl: process.env.LOGIN_SHELL_URL || "https://secure.adviser.qa.oldmutual.co.za/iicSeaUmsWeb/logoutRedirect.html",
    tokenKey: process.env.SSO_TOKEN_KEY || "token",
    userInfoKey: process.env.SSO_USER_INFO_KEY || "iicSeaUmsWeb-userInfo"
};
 ...
 ******************************************************************************/
     ...
     * lost.
     */
    credentialSecret: process.env.SECRET || "a-secret-key",
 ...
 ******************************************************************************/
	...
	adminAuth: {
		...
		users: [
			{
				username: process.env.OM_USER,
				password: process.env.OM_PASS,
				permissions: process.env.PERMISSION || "read"
			}
		],
		default: {
			permissions: process.env.PERMISSION || "read"
 ...
 ******************************************************************************/
     ...
     */
    httpStatic: path.resolve(process.cwd(), 'public'),
     ...
     */
    httpStaticRoot: '/nodered/',

    /** SSO entry guard for the editor UI. Follows the ai-web pattern:
     * read userInfo from browser storage and redirect to the external login shell
     * when missing. The runtime reads these values directly from environment
     * variables, so Docker/K8s can override them without rebuilding assets.
     */
    ssoGuard,
